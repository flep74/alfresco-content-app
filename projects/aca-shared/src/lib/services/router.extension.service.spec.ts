...
describe('RouterExtensionService', () => {
  ...
  describe('getApplicationRoutes', () => {
    ...
    it('should calculate the "component" to the registered component matching the "layout" value of the route', () => {
      extensionService.routes = [
        getDummyRoute({ layout: 'ext:components/about' })
      ];
    ...
    it('should calculate the "canActivateChild" and "canActivate" to default auth guard, if no "auth" defined for the route', () => {
      ...
      expect(service.getApplicationRoutes()[0].canActivateChild).toEqual([
        guard1
      ]);
    ...
    it('should calculate the "canActivateChild" and "canActivate" to the registered guard(s) matching the "auth" value of the route', () => {
      extensionService.routes = [
        getDummyRoute({ auth: ['ext.auth1', 'ext.auth2'] })
      ];
      ...
      expect(service.getApplicationRoutes()[0].canActivate).toEqual([
        guard2,
        guard3
      ]);
    });
    ...
    it('should calculate the main path and data of "children" with the component and data of the route', () => {
      const routeData = {};
      extensionService.routes = [
        getDummyRoute({ component: 'ext:components/about', data: routeData })
      ];

      expect(service.getApplicationRoutes()[0].children[0].path).toBe('');
      expect(service.getApplicationRoutes()[0].children[0].component).toBe(
        component1
      );
      expect(service.getApplicationRoutes()[0].children[0].data).toBe(
        routeData
      );
    });
    ...
    it('should calculate the "children"-s with the "children" value of the route', () => {
      ...
      expect(service.getApplicationRoutes()[0].children[0].outlet).toBe(
        'outlet1'
      );
      expect(service.getApplicationRoutes()[0].children[1].path).toBe(
        'child-path2'
      );
      expect(service.getApplicationRoutes()[0].children[1].component).toBe(
        component3
      );
      expect(service.getApplicationRoutes()[0].children[1].outlet).toBe(
        undefined
      );
      expect(service.getApplicationRoutes()[0].children[2].path).toBe('');
      expect(service.getApplicationRoutes()[0].children[2].component).toBe(
        component1
      );
    });
    ...
    it('should transform more routes, not just one', () => {
      extensionService.routes = [
        ...
        getDummyRoute({ path: 'aca:routes/login' })
      ];
  ...
  describe('mapExtensionRoutes', () => {
    ...
    it('should add routes to the right parent in reverse order (only one level deep searching)', () => {
      ...
      const parentRoute = router.config.find(
        routeConfig => routeConfig.path === parentRoutePath
      );
